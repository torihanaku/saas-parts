...
SaaS開発用の共有部品monorepo（全78パッケージ＝汎用部品70＋機能キット8）。失敗プロダクト dev-dashboard（torihanaku/dev-dashboard）から抽出・脱結合した実戦コード。全部品はテスト付き・自己完結（パッケージ間import無し・process.env読み取り無し・依存はすべて注入式）。
