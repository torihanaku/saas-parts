...
- `/Users/nakanomanabu/torihanaku/dev-dashboard-v2/server/lib/saml-sp.ts`（本体）
- `/Users/nakanomanabu/torihanaku/dev-dashboard-v2/server/routes/auth/saml-helpers.ts`（`buildSpMetadataXml` のみ移植）
- `/Users/nakanomanabu/torihanaku/dev-dashboard-v2/shared/types/sso.ts`（`SamlAssertion` 型）
- テスト: `/Users/nakanomanabu/torihanaku/dev-dashboard-v2/tests/saml-sp.test.ts`, `tests/saml-routes.test.ts`（buildSpMetadataXml の3ケースのみ）
