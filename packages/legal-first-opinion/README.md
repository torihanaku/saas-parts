...
契約・広告文の **AI ファーストオピニオン**（一次見解）を生成するキット。日本の 4 法令（薬機法・景品表示法・特定商取引法・個人情報保護法）について、法令ごとに「違反 / 非該当」の判定とその根拠（リスク指摘・確認観点）を返す。dev-dashboard-v2 `server/lib/legal/first-opinion.ts` の移植。
