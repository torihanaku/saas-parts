...
- `dev-dashboard-v2/shared/types/agency.ts`（93 LOC: role 階層型）
- `dev-dashboard-v2/server/middleware/require-client-access.ts`（115 LOC: #774 A-2）
- `dev-dashboard-v2/server/routes/agency-context.ts`（54 LOC）
- `dev-dashboard-v2/server/routes/team-members.ts`（#739 status healing → 純関数化）
