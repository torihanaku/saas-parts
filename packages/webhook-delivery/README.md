...
- `実運用SaaS/server/lib/webhook-delivery.ts`（本体）
- `実運用SaaS/server/lib/webhook-signing.ts`（`signPayload` のみインライン）
- 参考: `実運用SaaS/server/lib/job-scheduler.ts` の `webhookRetryHandler`（元プロジェクトの再送は外部cronで最大3試行。本パッケージではプロセス内 `deliverWithRetry()` として提供）
