...
describe("computeBm25Scores", () => {
  ...
  it("includes the matched skill names in each result", () => {
    const results = computeBm25Scores(baseInput());
    const c1 = results.find((r) => r.characterId === "c1")!;
    expect(c1.matchedSkills).toEqual(["rust"]);
  });

  it("penalizes characters with longer skill docs (BM25 normalization)", () => {
    const input = baseInput();
    // Same skill, same proficiency, but c1 has 10 skills (short) and c2 has 200 skills (long).
  ...
  it("accumulates score across multiple matched skills", () => {
    const input = baseInput();
    input.matchedSkillsByChar = new Map([
      ["c1", [
      ...
      ["c2", [{ name: "rust", proficiency: "expert" }]],
    ]);
  ...
  it("falls back to IDF=1.0 when a skill is missing from the idf map", () => {
    const input = baseInput();
    input.matchedSkillsByChar = new Map([
      ["c1", [{ name: "unknown-skill", proficiency: "expert" }]],
    ]);
    input.idfBySkill = new Map();
    const results = computeBm25Scores(input);
  ...
  it("falls back to proficiency weight 1.0 for unknown proficiency labels", () => {
    const input = baseInput();
    input.matchedSkillsByChar = new Map([
      ["c1", [{ name: "rust", proficiency: "mystery-level" }]],
    ]);
    const results = computeBm25Scores(input);
    expect(results[0]!.score).toBeGreaterThan(0);
  });
  ...
  it("golden: expert vs beginner scores at avg doc length match closed form", () => {
    // docLen == avgSkillCount → tfNorm = (k1+1)/(1+k1) = 2.5/2.5 = 1
    // c1 (expert):   3.0 × 1 × 1.25 = 3.75
    // c2 (beginner): 3.0 × 1 × 0.6  = 1.8
    const results = computeBm25Scores(baseInput());
    expect(results[0]!.score).toBeCloseTo(3.75, 10);
    expect(results[1]!.score).toBeCloseTo(1.8, 10);
  });
});
