...
出典: `dev-dashboard-v2` の `sentry/` `e2e/` `ops/` `.github/workflows/`。製品固有のブランド名・チャンネル名・プロジェクト slug は `{{PLACEHOLDER}}` に置換済み。秘密情報・実プロジェクト ID は含みません（E2E 内のトークンは元から `test-bypass-token` 等の**偽のテスト用フォールバック値**で、実値は環境変数から注入する設計）。
