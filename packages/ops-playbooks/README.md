...
出典: `実運用SaaS` の `sentry/` `e2e/` `ops/` `.github/workflows/`。製品固有のブランド名・チャンネル名・プロジェクト slug は `{{PLACEHOLDER}}` に置換済み。秘密情報・実プロジェクト ID は含みません（E2E 内のトークンは元から `test-bypass-token` 等の**偽のテスト用フォールバック値**で、実値は環境変数から注入する設計）。
...
└── workflows/
    └── security-check.yml           # PR/push/merge_group で走る secret スキャン＋リポ設定検証（GitHub Actions）
```
...
`main` への push / PR / merge_group（マージキュー）で走る GitHub Actions（約 92 行）。2 ジョブ構成:

1. **secret-scan** — gitleaks（v8.22.1 固定）で secret 漏洩を検出。PR は base..head の差分のみ、merge_group はキューの差分のみ、push は全履歴をスキャンと、イベント別に範囲を切り替えます。
2. **config-check** — 必須ファイル（`.gitignore` / `.editorconfig` / `.env.example` / ランタイムバージョンファイル）の存在検証と、二重ロックファイル・ワークフロー内の `|| true` によるエラー握りつぶし・AWS/API キーのハードコード（`AKIA…` / `sk-…`）の検出。

`.github/workflows/` にそのまま置けます。**プレースホルダなし**（元ファイルにプロジェクト ID・リポ名・組織名・サービス名・Slack チャンネルは含まれておらず、gitleaks バージョンと必須ファイル名だけが可変値）。依存監査は冗長を避けるため weekly schedule へ分離する運用を推奨（ファイル末尾の NOTE 参照）。自プロジェクトで不要な必須ファイル判定（Python 系など）は該当ブロックを削れば調整可能。
...
- ブランド名 `Folia` → `{{APP_NAME}}`、localStorage 接頭辞 `techradar` → `{{APP_SLUG}}`、Sentry project `実運用SaaS` / Slack `#ops`・`#marketing-ops` / owner `team:marketing-ops` → 各プレースホルダに置換。
...
`実運用SaaS` の `sentry/alert-rules/`・`e2e/`・`ops/`。
