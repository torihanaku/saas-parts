...
SaaS開発用の共有部品集（npm workspaces monorepo）。公開停止した自社プロダクト dev-dashboard（TS/TSX 約27.5万行）から、汎用性の高い実戦コードを抽出・脱結合したもの。**全112パッケージ＝汎用96＋機能キット10＋テンプレート6**。**dev-dashboardの全機能を抽出完了**（3体エージェントの最終総ざらいで取りこぼしゼロを確認・約3,300テストgreen）。残置分の台帳は [docs/not-extracted.md](./docs/not-extracted.md)。
...
| **依存脆弱性**（osv-scanner＋Dependabot） | 既知脆弱性のある依存 | 自動＋週次PR | 常時 |
| **ミューテーションテスト**（Stryker・`bun run mutation`） | 「テストが実はバグを検知できていない」穴 | 週次＋手動 | パイロット |
| **CodeQL**（`codeql.yml`） | データフロー/taint解析 | 手動（下記） | 要有効化 |

- **CodeQL と Security タブ(SARIF)連携**は private repo では GitHub Advanced Security が必要。リポを public にするか GHAS を有効化したら `codeql.yml` の `on:` と各スキャナの SARIF アップロードを有効化する。
