...
SaaS開発用の共有部品集（npm workspaces monorepo）。公開停止した自社プロダクト dev-dashboard（TS/TSX 約27.5万行）から、汎用性の高い実戦コードを抽出・脱結合したもの。**全111パッケージ＝汎用95＋機能キット10＋テンプレート6**（バッチ1=18・2=19・3=31・4=10・5=32）。**dev-dashboardの全機能を抽出完了**（約3,300テストgreen）。残置分の台帳は [docs/not-extracted.md](./docs/not-extracted.md)。
