...
SaaS開発用の共有部品集（npm workspaces monorepo）。公開停止した自社プロダクト dev-dashboard（TS/TSX 約27.5万行）から、汎用性の高い実戦コードを抽出・脱結合したもの。**全78パッケージ＝汎用70＋機能キット8**（バッチ1=18・バッチ2=19・バッチ3=31・バッチ4=10）。全数を機械監査して抽出済み（残置分の台帳は [docs/not-extracted.md](./docs/not-extracted.md)）。
